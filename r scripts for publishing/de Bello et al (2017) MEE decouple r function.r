...
# Methods in Ecology and Evolution, 8(10), 1200-1211.
...
decouple <- function(traits, 
  ...
  fdist <- scales::rescale(trait2dist(traits, ...)[[1]])
  ...
  if(kept[1] == "none") {
    ...
    dc_fdist <- scales::rescale(rda_traits[[1]])
    dc_pdist <- scales::rescale(rda_tree[[1]])
    covar_dist <- scales::rescale(rda_traits[[2]])
