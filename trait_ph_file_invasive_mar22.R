...
fd_env$Type<-as.factor(fd_env$Type)
fd_env<-fd_env%>% drop_na(Type)

# Foragers vs. Nesters
# in both secondary and primary
fd_test<-subset(fd_env, Type!="all")
  
tests_fd <- list()
tests_fd[[1]] <- t.test(Spines~Type, data=fd_test, paired=F) # **
tests_fd[[2]] <- t.test(Scul~Type,data=fd_test, paired=F) # ns
tests_fd[[3]] <- t.test(Rel.HW~Type,data=fd_test, paired=F) # ***
tests_fd[[4]] <- t.test(Rel.CL~Type,data=fd_test, paired=F) # ns
tests_fd[[5]] <- t.test(Rel.EP~Type,data=fd_test, paired=F) # ***
tests_fd[[6]] <- t.test(Rel.EW~Type,data=fd_test, paired=F) # ***
tests_fd[[7]] <- t.test(Rel.TL~Type,data=fd_test, paired=F) # ***
tests_fd[[8]] <- t.test(Rel.EP~Type,data=fd_test, paired=F) # ***
tests_fd[[9]] <- t.test(Rel.EW~Type,data=fd_test, paired=F) # ***
tests_fd[[10]] <- t.test(HL ~Type,data=fd_test, paired=F) # ***
tests_fd[[11]] <- t.test(poly.index ~Type,data=fd_test, paired=F) # ***
tests_fd[[12]] <- t.test(RaoQ~Type,data=fd_test, paired=F) # ***

### Primary only
tests_fd_prim <- list()
tests_fd_prim[[1]] <- t.test(Spines~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[2]] <- t.test(Scul~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[3]] <- t.test(Rel.HW~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[4]] <- t.test(Rel.CL~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[5]] <- t.test(Rel.EP~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # *
tests_fd_prim[[6]] <- t.test(Rel.EW~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # *
tests_fd_prim[[7]] <- t.test(Rel.TL~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[8]] <- t.test(Rel.EP~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # *
tests_fd_prim[[9]] <- t.test(Rel.EW~Type, paired=F, data=subset(fd_test, Forest=="Primary")) #  *
tests_fd_prim[[10]] <- t.test(HL ~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ***
tests_fd_prim[[11]] <- t.test(poly.index ~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ns
tests_fd_prim[[12]] <- t.test(RaoQ~Type, paired=F, data=subset(fd_test, Forest=="Primary")) # ***


### Secondary only
tests_fd_sec <- list()
tests_fd_sec[[1]] <- t.test(Spines~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[2]] <- t.test(Scul~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ns
tests_fd_sec[[3]] <- t.test(Rel.HW~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[4]] <- t.test(Rel.CL~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[5]] <- t.test(Rel.EP~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[7]] <- t.test(Rel.TL~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[8]] <- t.test(Rel.EP~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[9]] <- t.test(Rel.EW~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ns
tests_fd_sec[[10]] <- t.test(HL ~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[11]] <- t.test(poly.index ~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***
tests_fd_sec[[12]] <- t.test(RaoQ~Type, paired=F, data=subset(fd_test, Forest=="Secondary")) # ***

## 
tests <- list()
tests[[1]] <- t.test(cwm_tree$Spines~cwm_tree$Forest, paired=F)
tests[[2]] <- t.test(cwm_tree$Scul~cwm_tree$Forest, paired=F)
tests[[3]] <- t.test(cwm_tree$Rel.HW~cwm_tree$Forest, paired=F)
tests[[4]] <- t.test(cwm_tree$Rel.CL~cwm_tree$Forest, paired=F)
tests[[5]] <- t.test(cwm_tree$Rel.EP~cwm_tree$Forest, paired=F)
tests[[6]] <- t.test(cwm_tree$Rel.EW~cwm_tree$Forest, paired=F)
tests[[7]] <- t.test(cwm_tree$Rel.TL~cwm_tree$Forest, paired=F)
tests[[8]] <- t.test(cwm_tree$HL ~cwm_tree$Forest, paired=F)
tests[[9]] <- t.test(cwm_tree$poly.index ~cwm_tree$Forest, paired=F)
